...
export async function POST(request: NextRequest) {
  ...
  return withErrorHandler(async () => {
    ...
    if (!rawData && jobId) {
      ...
      if (!rawData) {
        return NextResponse.json(
          { error: 'Collection data not found' },
          { status: 404 }
        );
      }
    }
    ...
    if (!rawData || rawData.length === 0) {
      return NextResponse.json(
        { error: 'No data to generate articles from' },
        { status: 400 }
      );
    }
    ...
    for (const content of rawData) {
      try {
        ...
        // 興味度計算
        const interestScore = await interestCalculator.calculate(
          content,
          user.profile as any
        );
        if (interestScore < 3) {
    ...
    // 高品質記事のみ通知
    const highQualityArticles = generatedArticles.filter(
...
export class ArticleGenerator {
    ...
    try {
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'あなたは技術系ブログのライターです。提供された情報から魅力的な記事を作成してください。'
          },
          {
            role: 'user',
            content: prompt
  ...
  private buildPrompt(
    ...
    return ARTICLE_GENERATION_PROMPT
      .replace('{{user.techLevel}}', userProfile.techLevel)
...
export class GoogleSearchCollector extends BaseCollector {
  ...
  async collect(query: string, limit: number): Promise<RawContentData[]> {
    ...
    try {
      ...
      return items.map((item: GoogleSearchResult) => 
        this.transformToRawContent(item)
      );
  ...
  protected transformToRawContent(item: GoogleSearchResult): RawContentData {
    const publishedAt = item.pagemap?.metatags?.[0]?.['article:published_time'] 
