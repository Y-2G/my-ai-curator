...
const envSchema = z.object({
  ...
  // Security
  INTERNAL_API_KEY: z.string().min(32, 'INTERNAL_API_KEY must be at least 32 characters for security'),
  ...
  ENCRYPTION_KEY: z.string().min(32, 'ENCRYPTION_KEY must be at least 32 characters for security').optional(),
  
  // Security settings
  ALLOWED_ORIGINS: z.string().transform(val => val ? val.split(',').map(s => s.trim()) : []).optional(),
...
const parseEnv = () => {
  ...
  if (!parsed.success) {
    ...
    // 開発環境でのダミー値（警告と共に使用）
    const devDefaults = {
      POSTGRES_URL: process.env.POSTGRES_URL || 'postgresql://postgres:postgres@localhost:5432/smart_curator',
      POSTGRES_URL_NON_POOLING: process.env.POSTGRES_URL_NON_POOLING || 'postgresql://postgres:postgres@localhost:5432/smart_curator',
